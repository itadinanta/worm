...
public:
	...
	void AddToken(const mString & key, int tok) {
		insert(value_type(key, tok));
	};
	int GetToken(const mString & key) {
		if (find(key) == end())
...
mString Scanner::escape(const mString & SS)
{
	mString result;
	mString SS2;
...
void Scanner::AddToken(const mString & symbol, int token)
{
...
ParserStream *Scanner::FindStream(const mString & name)
{
	for (list < ParserStream * >::iterator i = streams.begin();
	     i != streams.end(); i++)
		if ((*i) && name == (*i)->GetName())
...
mString Scanner::GetLastString()
{
	return GetLastWord();
	// mString w(GetLastWord());
	// if (Token(w)==TOKEN_STRING) return w.at(1,w.length()-2);
	// else return w;
};

bool Scanner::IsIdentifier(const mString & Symbol) const {
	return 0;
} 

int Scanner::Token(const mString & Symbol) const {
	long i;
	double d;
	if (Symbol.empty())
		return TOKEN_NULL;
...
mString Scanner::GetSymbol(int token)
{
	switch (token) {
	case TOKEN_INT:
...
mString Scanner::literal(const mString & in)
{
